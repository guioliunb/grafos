...
list<vector<int>> result_cliques;
		...
		void Grafo::insertNodes(std::vector<std::string> &out){
			int aux = 1; 
		...
		void Grafo::tokenize(std::string const &str, const char delim,
		...
		void Grafo::BronKerbosch(vector<int> R, vector<int> P, vector<int> X, vector<vector<int>>& graph){
	  if (P.empty() && X.empty()){
